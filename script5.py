...
def create_json_object(list_elements):
    ...
    for element in list_elements:
        ...
        if element_id:
            # Create a dictionary with class names of child elements (excluding 'submitter') as keys and content as values
            class_content_dict = {}
            for child_element in element.find_all(recursive=False):
                class_names = ' '.join(child_element.get('class', []))
                content = child_element.text.strip()
                # Exclude elements with the class name 'submitter'
                if 'submitter' not in class_names:
                    class_content_dict[class_names] = content
